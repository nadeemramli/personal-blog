...
> - **Part 2 — The Base:**
> 	- **Part 2.0 (this article):** [[Part 2.0 - The Behavioral Base|The Behavioral Base]] — the dopamine reset, the inputs, and habit-stamping
...
This is the sequence that makes everything downstream work. It comes straight from the original sketch for this series, and the logic is the same staged build the [[Part 1.0 - The Cognitive Architecture|architecture]] implies: reset the hardware, then add performance, then add preservation.

### Step 1 — Drain the baseline (≈ one week)
...
- **No doomscrolling / short-form video.** The single biggest lever. Short-form feeds are the most efficient dopamine-deflation machine ever built.
...
- **Kill background novelty.** Constant music, constant podcasts, a tab always open "just in case" — these are low-grade drips. Let yourself be a bit bored. Boredom is the *return* of sensitivity, not a failure state.
...
---
## The four hardware inputs no compound replaces

Before any molecule, four free inputs determine whether your brain *can* perform. The [[Part 1.1 - Measuring Cognition|sleep experiment from Part 1.1]] will have already shown you that the gap between rested and unrested dwarfs anything in Part 3.
...
### 4. Cognitive load management — protecting the attention you have
...
- **Single-task by default.** The deep-work minutes from [[Part 1.1 - Measuring Cognition|Part 1.1]] only accumulate if nothing interrupts them.
...
> - **Measurement:** you have 2–3 weeks of leading-metric and PR baseline data from [[Part 1.1 - Measuring Cognition|Part 1.1]], so you can actually *evaluate* anything you add.
...
> - **The three-step reset:** drain the baseline (~a week off the cheap hits, greyscale phone) → attach reward to real work (add a stimulant *here*, if at all) → consolidate with slow preservation agents.
> - **Four free inputs beat any stack:** sleep (upstream of everything), cardio (BDNF), hydration/stable fuel, and cognitive-load management (single-task, phone out of the room).
...
2. **Fix the sleep first.** If only one thing changes this month, make it a fixed wake time allowing 7–9 hours. Re-run the [[Part 1.1 - Measuring Cognition|rested-vs-tired PR test]] and keep the delta visible.
...
5. **Check the readiness gate** before reading Part 3 as a buyer rather than a student. If you can't tick the boxes, the highest-return move is still behavioural.
