---
title: Part 6.0 — Just-in-Time Project Management
draft: false
tags:
  - productive
  - productivity
...
date: 2026-05-30
---
> [!abstract] This is **Part 6 of 7** in the Productivity Enhancement Series
>
...
> 	- **Part 6.0 (this article):** [[Part 6.0 - Just-in-Time Project Management|Just-in-Time Project Management]] (pull a project through the system only when it's due)
...
- [The problem with carrying every project at once](#the-problem-with-carrying-every-project-at-once)
- [Just-in-time, borrowed from the factory floor](#just-in-time-borrowed-from-the-factory-floor)
- [Just-in-time vs just-in-case](#just-in-time-vs-just-in-case)
- [How the system pulls a project forward](#how-the-system-pulls-a-project-forward)
- [Running it: a few active, the rest dormant](#running-it-a-few-active-the-rest-dormant)
- [Part 6 Takeaways](#part-6-takeaways)
- [Your JIT Task List](#your-jit-task-list)
- [Sources & references](#sources--references)

---

> [!important] The problem with carrying every project at once
> By now the stack works: the [[Part 3.0 - The Workflow Engine|workflow]] catches everything and the [[Part 4.0 - PARA|knowledge system]] feeds it. The failure that's left is *scope*. People try to make progress on every project they've ever started, all at once, and the result is forty half-open loops, each generating a low hum of guilt, none actually moving. ==Carrying every project at once is how you stay maximally busy and minimally finished.== Just-in-Time Project Management is the discipline of pulling a project into active work *only when it's genuinely due*, and leaving the rest dormant without guilt. It's the last term this series keeps from the original framework[^jit], and it's the one that fixes overwhelm.

---
## Just-in-time, borrowed from the factory floor

The name comes from manufacturing. The old way (just-in-*case*) was to stockpile huge inventories of parts so you'd never run short. It tied up capital, filled warehouses, and most of the stock sat idle and went obsolete. Toyota's just-in-time approach flipped it: produce and pull parts *only as demand calls for them*, holding almost no inventory.[^toyota] Less waste, less clutter, more responsiveness.

Your projects are the inventory. ==The just-in-case mind stockpiles open projects "so they're ready"; the just-in-time mind keeps almost all of them dormant and pulls one forward only when its moment arrives.== The dormant projects aren't abandoned, they're [[Part 4.0 - PARA|archived or parked]], costing you nothing, ready to activate when their deadline actually approaches. You hold a tiny "inventory" of active work and let real demand (a deadline, an opportunity) pull the next one in.
...
**Just-in-time learning** is learning the thing *when a live project demands it*. On-the-job, pulled by a real problem. You learn the specific React hook when the feature needs it, the specific clause when the contract hits your desk. It sticks because it's immediately used, and it's never wasted because you only ever learn what the work actually pulled for.[^learning]

This is why JIT project management and the [[Part 1.0 - How Learning Actually Happens|Learning series]] are two faces of one principle: ==let real demand pull the work and the learning forward, instead of stockpiling either against a hypothetical future.== Most "I should get ready" effort is just-in-case waste.
...
1. **The project activates** (a deadline approaches, or you deliberately pull it from the parked list).
2. **You pull its Resources just-in-time.** Everything you filed under it in [[Part 4.0 - PARA|PARA]], plus relevant [[Part 4.1 - Progressive Summarization|maps]], comes into the active project. You're not starting from a blank page; you're starting from everything past-you already gathered.
3. **You learn what it pulls for.** Any skill gap the project exposes, you fill *now*, on the job, because now it'll stick.
4. **You define the next action** ([[Part 3.0 - The Workflow Engine|Part 3.0]]) and execute, single project in focus.
5. **On completion, the whole thing sweeps to Archives**, and the active inventory drops back down, ready to pull the next one.

> [!tip] The AI multiplier on JIT
> This is where AI earns its place at the top of the stack. When you pull a project forward, an assistant can *assemble the kit*: gather the relevant resources and maps, draft the project plan and the first next action, and summarise what you already know about the problem. The blank-page cost of starting a project drops to near zero, which is exactly what removes the resistance that made you stockpile in the first place. And a fully repeatable project type can be handed to an [[Part 7.0 - The Agentic AI Framework|agent]] to run, which is Part 7.
...
- **Parked is a real, guilt-free state.** A parked project isn't a failure; it's inventory waiting for demand. Reviewing the parked list is part of your [[Part 3.0 - The Workflow Engine|weekly review]]: does anything need to activate yet? Usually the answer is no, and that's fine.
...
[^jit]: "Just-in-Time Project Management" is Tiago Forte's term, kept from the source framework. It reframes project work as pulling resources and effort forward only as a project genuinely demands them, rather than maintaining many projects in parallel. It sits at the top of the productivity stack because it depends on the [[Part 3.0 - The Workflow Engine|workflow]] and [[Part 4.0 - PARA|knowledge]] layers being in place.
[^toyota]: The just-in-time manufacturing method (the Toyota Production System) minimises inventory by producing and supplying parts only as downstream demand pulls them, reducing waste and holding cost. It's the source analogy for pulling projects and learning on demand rather than stockpiling them.
[^learning]: This connects to the [[Part 1.0 - How Learning Actually Happens|Learning series]]: knowledge acquired in response to a concrete, current problem is retained far better than knowledge studied "just in case," because it's immediately retrieved and applied. The note that pre-built curricula fail to match your live on-the-job problem is the learning-side statement of the same just-in-time principle.
