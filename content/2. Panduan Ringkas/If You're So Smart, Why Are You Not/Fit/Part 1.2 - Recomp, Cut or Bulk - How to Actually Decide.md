---
title: Part 1.2 - Recomp, Cut or Bulk - How to Actually Decide
draft: false
tags:
  - fitness
date: 2026-04-29
...
- [Why bulking is overrated for naturals](#why-bulking-is-overrated-for-naturals)
- [The four phases: When each one applies](#the-four-phases-when-each-one-applies)
    - [Recomposition — the default](#recomposition--the-default)
    - [Cut — when you "almost see your abs"](#cut--when-you-almost-see-your-abs)
    - [Lean bulk — only after the abs are visible](#lean-bulk--only-after-the-abs-are-visible)
    - ["Bulk" — basically never, for naturals](#bulk--basically-never-for-naturals)
- [The decision tree](#the-decision-tree)
- [The two rules underneath everything](#the-two-rules-underneath-everything)
    - [Rule 1: Understand "true weight"](#rule-1-understand-true-weight)
    - [Rule 2: Understand insulin activation](#rule-2-understand-insulin-activation)
- [Finding maintenance calories — the 2-week lock-in](#finding-maintenance-calories--the-2-week-lock-in)
    - [What is TDEE?](#what-is-tdee)
    - [The lock-in protocol](#the-lock-in-protocol)
- [The Fitness Forecast](#the-fitness-forecast)
- [Part 1.2 Takeaways](#part-12-takeaways)
- [Your Week 1–2 Task List](#your-week-12-task-list)
---
[[Part 1.1 - What Actually Matters|Part 1.1]] established the _what_, what gets measured, *(i.e why FFMI matters more than scale weight)*, and why strength is the natural lifter's most honest proxy for size. This article is the _how_: given your current numbers, how do you decide whether to recomp, cut, or bulk and how do you actually find the calorie anchor that makes any of those decisions executable?
...
> [!NOTE]
> More about Maintenance Calories at [Finding maintenance calories](#finding-maintenance-calories--the-2-week-lock-in) below.
...
> [!warning] The trap Three months eating big, gain 4 kg of muscle and 6 kg of fat. Then four months cutting and lose the 6 kg of fat plus 2 of the 4 kg of muscle.
> 
...
This is roughly the [10–20% oscillation](https://outlift.com/bulk-or-cut/) that most experienced naturals settle into. Most of the year is spent somewhere in that band, alternating between mild surplus and mild deficit, with the long-term trajectory pointing toward more muscle at the same body fat over years.
...
> [!tip] Definition **True weight is the rolling 7-day average of morning weigh-ins.** That's the number that tracks reality. Compare _this_ week's average to _last_ week's average to know whether you're actually gaining, losing, or maintaining.
...
The principle: big insulin spikes belong at moments when muscles are hungry for glucose (around training, around movement). Low insulin baseline belongs the rest of the time. That combination is what produces both good body composition _and_ good metabolic health long-term.

> [!info] The GLUT4 mechanism GLUT4 is a glucose transporter that lives inside muscle cells and only moves to the cell surface when called. Two things call it: **insulin** (after eating) and **muscle contraction** (training, walking, even air squats).
> 
...
> [!important] Two weeks is the minimum lock-in period The first 7 days establish baseline weight trend. The second 7 days confirm whether the trend is actually flat, rising, or falling. Anything shorter and the noise from water/glycogen/bowel weight will swamp the signal.
...
|Week 2 vs. Week 1 average|What it means|Adjustment|
|---|---|---|
|Flat (within ±0.3 kg)|Calorie number is maintenance|Lock it in|
|Dropped > 0.3 kg|Estimate was below maintenance|True TDEE ≈ current intake + (kg lost × 7,700) ÷ 14|
|Climbed > 0.3 kg|Estimate was above maintenance|True TDEE ≈ current intake − (kg gained × 7,700) ÷ 14|

(7,700 kcal ≈ 1 kg of body weight. Useful conversion to remember.)

> [!warning] Two non-negotiables during the lock-in
> 
...
> [!tip] MacroFactor automates this MacroFactor's expenditure algorithm recalculates true maintenance every week based on what was eaten and what the weight trend did, removing the manual math entirely.
...
Now that you have your baseline from [[Part 1.1 - Recomp, Cut, or Bulk - What Actually Matters|Part 1.1]], it's time to put in the work. You cannot pick a direction—Bulk, Cut, or Recomp—if you don't know where "stationary" is.

Your goal is to have your **True Maintenance Calories** locked in by the end of **Week 2**. Once you have that anchor, you aren't just guessing; you are forecasting.

Use this simple template to visualize your trajectory once your maintenance is found:

|**Week**|**Phase**|**Calorie Delta**|**Predicted BF%**|**Performance Goal**|
|---|---|---|---|---|
|1-2|Lock-in|±0\pm 0 ±0 (Maintenance)|Baseline|Form Mastery|
|4|Execution|+200+200 +200 or −500-500 −500|−0.5%-0.5\% −0.5% or +0.1%+0.1\% +0.1%|+2.5+2.5 +2.5 kg on Main Lift|
|8|Review|Adjusted|Trend Analysis|New Rep PR|
...
> - **Rule 1 — True weight:** Daily weight is noise. The 7-day rolling average is signal. Compare _averages_, never single days.
...
Before moving on to [[Part 2.0 - Structure of a Day|Part 2]], lock in your maintenance number. Without this anchor, every later decision is guesswork.
...
5. **Apply the adjustment formula** — Use the table in [The lock-in protocol](#the-lock-in-protocol) to adjust to true maintenance. That number is your anchor for the next 6–8 weeks.
...
7. **Pick your phase** — Use the [decision tree](#the-decision-tree) and your DEXA/InBody body fat number from [[Part 1.1 - Recomp, Cut, or Bulk - What Actually Matters|Part 1.1]] to choose recomp, cut, or lean bulk. Document the choice.

> [!note] Up next For how to structure the day around all this — when to eat, when to train, when to walk, how to track — go to [[Part 2.0 - Structure of a Day|Part 2 — Structure of a Day]].
> 
...
1. On natural lifter cut strategy and the muscle catabolism risk, see [outlift.com — bulk or cut](https://outlift.com/bulk-or-cut/) and [rippedbody.com — cut or bulk](https://rippedbody.com/cut-or-bulk/). [↩](#user-content-fnref-10)
    
2. Weight trend smoothing approach used by [MacroFactor](https://macrofactorapp.com/) and similar apps; underlying principle of using rolling averages to extract signal from noisy daily weights is standard in body composition tracking. [↩](#user-content-fnref-1)
    
3. GLUT4 mechanism: muscle contraction activates GLUT4 translocation independently of insulin, producing additive effect when both signals fire. See review at [PubMed Central — PMC8372753](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC8372753/) and overview at [bellyproof.com — advanced hypertrophy mechanics](https://bellyproof.com/science/advanced-hypertrophy-mechanics/). [↩](#user-content-fnref-2)